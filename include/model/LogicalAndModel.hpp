 ...
 * 
 * 输出端口：
 * - 0: BooleanData - A AND B
 */
class LogicalAndModel : public BaseNodeModel
{
    Q_OBJECT
...
public:
    ...
    std::shared_ptr<QtNodes::NodeData> outData(QtNodes::PortIndex port) override {
        ...
        QString description = QString("%1 AND %2 = %3")
    ...
    QWidget* embeddedWidget() override {
        if (!m_widget) {
            ...
            auto titleLabel = new QLabel("AND");
    ...
    bool createPropertyPanel(PropertyWidget* propertyWidget) override {
        ...
        propertyWidget->addTitle("逻辑与运算");
        propertyWidget->addDescription("只有当两个输入都为true时，输出才为true");

        propertyWidget->addInfoProperty("运算符", "AND (∧)");
        propertyWidget->addInfoProperty("真值表", "True AND True = True\n其他情况 = False");
